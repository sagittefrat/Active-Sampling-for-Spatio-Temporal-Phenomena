...
attributes=[time, waypoints, score, expanded]


def main(problem_type='spirals', precent_train=0.1, time_to_reach=1800):
	...
	results_df=pd.DataFrame(columns = attributes[:-1]+ ['mode'])
	df_score_points=pd.DataFrame(columns = attributes[:-1]+ ['mode'])
	...
	for file in sorted(os.listdir(problem_folder_name)):
		...
		df=pd.read_csv(os.path.join(problem_folder_name, file), index_col=False, dtype = {waypoints : "float64"})[attributes]
		...
		df_score_points=df_score_points.append(df, sort=True)
	...
	df_rand=pd.DataFrame( zip(res_rand_waypoints, res_rand_score, res_rand_expanded), index=res_rand_score.index, columns = attributes[1:])
	df_gree=pd.DataFrame( zip(res_gree_waypoints, res_gree_score, res_gree_expanded), index=res_gree_score.index, columns = attributes[1:])
	df_look=pd.DataFrame( zip(res_look_waypoints, res_look_score, res_look_expanded), index=res_look_score.index, columns = attributes[1:])
