...
class CreateVOIEvaluator(object):
	...
	def find_objective_properties(self, data, database, k_dist=10, l_time=3, alpha=0.5):
		...
		for objective in range(0,data.num_objectives):
			...
			k_nearest_neigh_by_place=self.find_k_nearest_neigh_by_place(objective_coor, k_dist, database)
			...
			objective_value = self.classifier(l_neareset_neigh_by_time, k_nearest_neigh_by_place, database)
			objective_voi = self.voi_eval(l_neareset_neigh_by_time, objective)
	...
	def voi_eval(self, l_neareset_neigh_by_time,objective, alpha=0.5):
		if objective==0: return 0
		to_compare_time=[i[1] for i in l_neareset_neigh_by_time ]
